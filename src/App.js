...
const all_champs = ["Aatrox", "Ahri", "Akali", "Akshan", "Alistar", "Amumu", "Anivia", "Annie", "Aphelios", "Ashe", "AurelionSol", "Azir", "Bard", "BelVeth", "Blitzcrank", "Brand", "Braum", "Caitlyn", "Camille", "Cassiopeia", "ChoGath", "Corki", "Darius", "Diana", "Draven", "DrMundo", "Ekko", "Elise", "Evelynn", "Ezreal", "Fiddlesticks", "Fiora", "Fizz", "Galio", "Gangplank", "Garen", "Gnar", "Gragas", "Graves", "Gwen", "Hecarim", "Heimerdinger", "Illaoi", "Irelia", "Ivern", "Janna", "JarvanIV", "Jax", "Jayce", "Jhin", "Jinx", "KSante", "KaiSa", "Kalista", "Karma", "Karthus", "Kassadin", "Katarina", "Kayle", "Kayn", "Kennen", "KhaZix", "Kindred", "Kled", "KogMaw", "LeBlanc", "LeeSin", "Leona", "Lillia", "Lissandra", "Lucian", "Lulu", "Lux", "Malphite", "Malzahar", "Maokai", "MasterYi", "MissFortune", "Mordekaiser", "Morgana", "Nami", "Nasus", "Nautilus", "Neeko", "Nidalee", "Nilah", "Nocturne", "Nunu", "Olaf", "Orianna", "Ornn", "Pantheon", "Poppy", "Pyke", "Qiyana", "Quinn", "Rakan", "Rammus", "RekSai", "Rell", "RenataGlasc", "Renekton", "Rengar", "Riven", "Rumble", "Ryze", "Samira", "Sejuani", "Senna", "Seraphine", "Sett", "Shaco", "Shen", "Shyvana", "Singed", "Sion", "Sivir", "Skarner", "Sona", "Soraka", "Swain", "Sylas", "Syndra", "TahmKench", "Taliyah", "Talon", "Taric", "Teemo", "Thresh", "Tristana", "Trundle", "Tryndamere", "TwistedFate", "Twitch", "Udyr", "Urgot", "Varus", "Vayne", "Veigar", "VelKoz", "Vex", "Vi", "Viego", "Viktor", "Vladimir", "Volibear", "Warwick", "Wukong", "Xayah", "Xerath", "XinZhao", "Yasuo", "Yone", "Yorick", "Yuumi", "Zac", "Zed", "Zeri", "Ziggs", "Zilean", "Zoe", "Zyra"]
...
function App() {
  ...
  const [blockList, setBlockList] = useState([['Yuumi'], [], [], [], [], []]);
    ...
    <>
      ...
      <table>
        <caption>
          <div id='random_box'>
            <RandomCard {...random} />
            <button onClick={() => { randomChamp(); }} style={{ marginBottom: 20 }}>Random</button>

          </div>
          Role: <select onChange={e => setListNum(e.target.value)}>
            <option selected value={0}>All</option>
            <option value={1}>Support</option>
            <option value={2}>Bottom</option>
            <option value={3}>Mid</option>
            <option value={4}>Jungle</option>
            <option value={5}>Top</option></select>
          <br />

          <input placeholder='Search' value={searchText} style={{ height: '2em', width: '16em' }} type='text' onChange={e => setSearchText(e.target.value)}></input>

          <div>
            Champ Pool (Click to Exclude Champs)

            <button style={{ height: '35px', marginLeft: '62.5%' }} onClick={() => blockAll()}>Block All</button>
          </div>

        </caption>

        <tbody>
          {ChunkArray(all_champs.filter(s => !blockList[listNum].includes(s)).filter(handleSearch).map(s => makeChamp(s)), cols).map(row => <ChampRow champs={row} />)}
        </tbody>
      </table>
      <br></br>
      <table>
        <caption >
          <span>
            Blocked Champs (Click to Return to Champ Pool):<button style={{ height: '35px', marginLeft: '56%' }} onClick={() => unblockAll()}>Unblock All</button></span>
        </caption>
        <tbody>
          {ChunkArray(blockList[listNum].filter(handleSearch).map(s => makeChamp(s)), cols).map(row => <ChampRow champs={row} />)}
        </tbody>
      </table>
    </>
  );
  ...
  function ChampRow({ champs }) {
    ...
    return (
      <tr >
        {champs.map((s, i) => <ChampCard {...s} key={s} />)}
        {champs > 1 ? Array(diff).fill('').map(s => <td style={{ width: '100%', hover: 'none' }}> </td>) : <td></td>}
      </tr >
    );
  }
